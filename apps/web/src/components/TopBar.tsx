  ...
  return (
    <header className="h-14 flex-shrink-0 bg-zinc-950 border-b border-zinc-800 flex items-center justify-between px-4 gap-4">
      <div className="flex items-center gap-2 min-w-0">
        {tier === "free" && (
          <span className="hidden sm:block text-xs text-amber-400 bg-amber-950/40 border border-amber-800 rounded px-2 py-0.5">
            Signals delayed 30 min —{" "}
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <span className="hidden md:block text-xs text-zinc-500 truncate max-w-[160px]">
          {user.email}
        </span>
        <button
          onClick={signOut}
